# D:\final\drugs\templatetags\jalali_tags.py
...
@register.simple_tag
def jalali_date_widget(field, is_date=True):
    from jalali_date.widgets import DatePicker, DateTimePicker
    return DatePicker() if is_date else DateTimePicker()
...
@register.filter(name='to_persian_words')
def to_persian_words_filter(value):
    try:
        return digits.to_word(value) 
    except (ValueError, TypeError):
        return value
    except Exception as e:
        print(f"Error converting {value} to persian words: {e}")
        return value